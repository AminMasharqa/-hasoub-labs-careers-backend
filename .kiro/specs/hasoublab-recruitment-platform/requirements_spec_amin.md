...
2. WHEN a Residency_Proof is submitted, THE Platform SHALL apply automated format validation: phone numbers SHALL match Israeli numbering (`+972` followed by 9 digits, or a local `0`-prefix followed by 9 digits); national ID numbers SHALL be exactly 9 digits and pass the Israeli ID check-digit algorithm; addresses SHALL contain a non-empty street name, a house or building number, and a city, within a 200-character field limit.
...
10. IF the Verification_Code expires before it is confirmed, THEN THE Platform SHALL set the Geographic_Verification record to `Expired` and apply the registration-expiry behavior in Requirement 1.
11. WHEN an Admin reviews an account prior to `Approved`, THE Platform SHALL present the submitted Residency_Proof and the Geographic_Verification state; the Admin MAY reject the account on residency grounds (Requirement 1).
12. WHEN an Admin approves residency despite failed or unverifiable automated format validation, THE Platform SHALL set the Geographic_Verification record to `ManualOverride` and record the Admin identity, a UTC timestamp, and a reason of at least 10 characters.
13. THE Platform SHALL deny all feature access, except the authentication, code-entry, and onboarding screens, to any account whose Geographic_Verification record is not `Verified` or `ManualOverride`.
...
2. THE Platform SHALL grant Admin sessions access to: user and role management, all Candidate profiles and CV_Versions, all Job_Descriptions in any status, all Applications and their status, the Audit_Log (read and search only), and platform configuration.
...
12. WHEN an Application is successfully submitted, THE Platform SHALL send the Candidate an in-app confirmation immediately and an email confirmation within 5 minutes, and SHALL notify an Admin and the Job_Description's creator.
...
* **Gate invariant**: Any account granted feature access has a Geographic_Verification record in `Verified` or `ManualOverride`.
* **Code precondition**: No account leaves `PendingVerification` without a confirmed Verification_Code (or an Admin `ManualOverride`).
